...
contract Escrow {
    ...
    constructor(uint256 _value) {
        ...
        // Guardando o endereço de quem fez deploy do contrato
        value = _value;
        // Guardando o valor do contrato
        ...
        // Guardando a data que o contrato foi para a rede
    }
    ...
    function buy() public payable avaliable {
        require(
            msg.value > 0,
        ...
        // Guarda o endereço do vendedor.
        balance += msg.value;
        // Adiciona o valor transferido ao saldo.
        buyDate = block.timestamp;
        // Guarda a data que foi feito o pagamento.
        emit Buy(buyer, msg.value);
        // Emite um evento de compra.
    }
    ...
    function addMoreBalance() public payable avaliable onlyBuyer {
        require(
            msg.value > 0,
            "O valor transferido precisa ser maior que zero"
        );
    ...
    function changeValue(uint256 _value) public avaliable onlySeller {
        require(
            buyer == address(0),
            "Valor so pode ser alterado caso nao tenha um comprador"
        );
        value = _value;
        // Muda o valor do escrow.
    }

    // Função que o vendedor avisa que o produto já foi enviado.
    function sendProduct() public avaliable onlySeller {
        require(balance >= value, "Saldo deve ser maior que o valor do escrow");
        sellerOk = true;
        // Adiciona status ok para o vendedor.
        emit Sent();
        // Emite um evento que o produto foi enviado.
    }
    ...
    function confirmProduct() public onlyBuyer {
        ...
        buyerOk = true;
        // Adiciona o status ok para o comprador.
        paySeller();
        // Chama a função que faz o pagamento para o vendedor.
    }
    ...
    function paySeller() private {
        ...
        // Faz o envio do balanço para o vendedor.
        if (ok == true) {
            // Verifica se o envio foi feito.
            balance = 0;
            // Zera o saldo.
            emit Finish();
            // Emite um evento de finalização do escrow.
    ...
    function refund() public onlySeller {
        ...
        if (ok) {
            ...
            // Zera o saldo.
            buyer = payable(0);
            // Remove o comprador para o produto ficar disponível novamente para compra.
            buyDate = 0;
            // Zera a data de compra.
            sellerOk = false;
            // Mudar o status o vendedor para não ok.
        }
    }
    ...
    function report() public onlyBuyer {
        ...
        if (buyer.send(balance)) {
            // Faz o envio do balanço para o comprador já verificando se o envio foi feito.
            balance = 0;
            // Zera o saldo.
            buyer = payable(0);
            // Remove o comprador para o produto ficar disponível novamente para compra.
